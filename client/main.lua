                    ...
                    Citizen.CreateThread(function()
                        while GetPedInVehicleSeat(tempTrain, -1) == PlayerPedId() do
                            ...
                            for x, w in pairs(Config.Stations) do
                                if GetDistanceBetweenCoords(GetEntityCoords(tempTrain), w.coordsdeletetrain, true) < 8 then
                                    wait2 = 0
...
Citizen.CreateThread(function()
    ...
    ModelsLoaded = true
	for k, v in pairs (Config.TrainShop) do
		v.blip = Config.CreateBlip(v.coordspnj.xyz, 0.5, Config.Lang["TrainShop"], 250, 45)
    ...
    for k, v in pairs (Config.Stations) do
		v.blip = Config.CreateBlip(v.coordspnj.xyz, 0.5, Config.Lang["TrainStation"], 250, 45)
        v.pnj = Config.CreatePNJ(v.coordspnj, v.pedmodel, false)
        Citizen.CreateThread(function()
            while true do
                local wait = 1000
