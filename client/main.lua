...
RageMenuTrain.Menu.Path =
    RageUI.CreateSubMenu(
    RageMenuTrain.Menu.Garage,
    "",
    Config.Lang["ChooseTrail"],
    nil,
    nil,
    "root_cause",
        ...
        RageUI.IsVisible(
            ...
            function()
                for k, v in pairs(ownedTrain) do
                    if
                        Config.Stations[index].metrostation ~= nil and Config.Stations[index].metrostation and
                            v.trainindex ~= Config.MetroIndex
                     then
                        goto skipThisTrain
                    end
                    if v.station == index then
                        RageUI.Button(
                            string.format(Config.Lang["GetOutTrain"], v.label),
                            string.format(Config.Lang["TrainInfos"], v.price, v.storage),
                            {},
                            v.state == "in",
                            {
                                onSelected = function()
                            ...
                            if
                             ...
                             then
                                ...
                                for i = 0, 100 do
                                    ...
                                    local backTrailer =
                                        GetOffsetFromEntityInWorldCoords(GetTrainCarriage(tempTrain, i), 0.0, -8.3, 1.0)
                                    ...
                                    if
                                        GetDistanceBetweenCoords(backTrailer, GetEntityCoords(PlayerPedId()), true) <
                                            3.0 and IsControlJustReleased(0, 38)
                                     then
